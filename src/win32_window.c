...
static void updateFramebufferTransparency(const _GLFWwindow *window) {
  ...
  if (IsWindows8OrGreater() ||
    ...
    SetLayeredWindowAttributes(window->win32.handle, 0x0030c100, 254,
                               LWA_ALPHA);
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_TOUCH: {
    ...
    TOUCHINPUT *touches = calloc(numInputs, sizeof(TOUCHINPUT));

    if (touches) {
      if (GetTouchInputInfo((HANDLE)lParam, numInputs, touches,
                            sizeof(TOUCHINPUT))) {
        // Only process single touch as mouse click
        if (numInputs == 1) {
          POINT pt = {TOUCH_COORD_TO_PIXEL(touches[0].x),
                      TOUCH_COORD_TO_PIXEL(touches[0].y)};
